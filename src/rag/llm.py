...
Azure OpenAI に必要な環境変数:
  AZURE_OPENAI_API_KEY          : APIキー
  AZURE_OPENAI_ENDPOINT         : エンドポイントURL（例: https://xxx.openai.azure.com/）
  AZURE_OPENAI_API_VERSION      : APIバージョン（例: 2024-02-01）
  LLM_MODEL                     : チャット用デプロイメント名（例: gpt-4o）
  AZURE_OPENAI_EMBEDDING_DEPLOYMENT: Embedding用デプロイメント名（例: text-embedding-3-small）
"""
...
def build_embeddings() -> Embeddings:
    ...
    Ollamaはembeddingモデルのサポートが限定的なため、
    Gemini embeddingをフォールバックとして使用する。
    ...
    # gemini または ollama（ollamaはGemini embeddingをフォールバック）
