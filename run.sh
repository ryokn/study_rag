...
show_menu() {
    ...
    echo "  5) table        CSVテーブル検索（DuckDB）"
    echo "  6) web          Web UI を起動"
    echo "  7) eval         品質評価（RAGAS + MLflow）"
    echo "  8) mlflow       MLflow UI を起動"
    echo "  q) quit         終了"
    echo ""
}
...
run_choice() {
    ...
    case $choice in
        ...
        5)
            printf "${YELLOW}CSVディレクトリ（未入力でデフォルト: data/csv）: ${RESET}"
            read -r csv_dir
            csv_dir=${csv_dir:-data/csv}
            uv run src/main.py table --csv-dir "$csv_dir"
            ;;
        6) uv run streamlit run src/app.py ;;
        7)
            ...
            ;;
        8) uv run mlflow ui --backend-store-uri sqlite:///mlruns.db ;;
        q|Q) echo "終了します。"; exit 0 ;;
        *) echo "無効な選択です。1〜8 または q を入力してください。" ;;
    esac
}
