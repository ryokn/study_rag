...
```bash
# 実験1: デフォルト設定
uv run python src/main.py eval --questions-file data/eval_questions.json --run-name "chunk500_topk5"

# 実験2: チャンクサイズを小さく
CHUNK_SIZE=200 uv run python src/main.py eval --questions-file data/eval_questions.json --run-name "chunk200_topk5"

# 実験3: 取得チャンク数を増やす
TOP_K=10 uv run python src/main.py eval --questions-file data/eval_questions.json --run-name "chunk500_topk10"
```
